...
void bmdp_t::createPyGrid(std::vector<Strmode> ver_valid, arma::mat
  ...
  std::string x_dim_str = std::to_string(x_dim);
  // 2. Number of states
  int nstates = 0;
  ...
  if (x_dim < 3) {
    ...
    std::cout << "Can not plot dimensions > 3!" << std::endl;
    return;
  }
  for (size_t i = 0; i < x_dim; i++) {
    myfile << "boundary" << i << "=( ";
...
void bmdp_t::formatOutput(double time, std::string cT) {
  ...
  std::string str("y");
  std::string str1("yes");
  //std::cout << "Would you like to store the results generated by IMDP [y- yes, n - no] " << std::endl;
  //std::cin >> exportOpt;
  std::cout << "Store the results generated by IMDP" << std::endl;
  exportOpt = "y";
  ...
  std::string str2("Y");
  std::string str3("YES");
  // std::cout << "Would you like to generate abstraction figure [y - yes, n - no]"
  //           << std::endl;
  // std::cin >> exportOpt;
  std::cout << "Generate abstraction figure" << std::endl;
  exportOpt = "Y";
